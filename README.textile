...
Handlers are very simple - in fact, you could probably get away
with registering Object.new as a handler.

To function, handlers only require one method: handle - which takes
two options. an event name (e.g. :incoming_message) and a hash
of the aformentioned attributes / details. This data can then be processed.
...
The other you may want to look at is Marvin::CommandHandler - you
can then just define methods and "expose" them - e.g.:
